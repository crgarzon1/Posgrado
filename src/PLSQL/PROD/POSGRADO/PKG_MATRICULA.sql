...
FFFFFFFFFFF           IIIIIIIIIIXXXXXXX       XXXXXXXMMMMMMMM               MMMMMMMMEEEEEEEEEEEEEEEEEEEEEE
    */
    ...
    -- los estudiantes 85191219, 85111231 entran en puedeVerTuroriaPostmalla().
    procedure getOferta(
        ...
        v_grs json_list;
        v_tipo_estudiante B_ESTUDIANTES.TIPO_DE_INGRESO%type;
    begin
        pkg_html.corsHeaders();
        admisiones.pkg_utils.getAnioCicloEsquema('2',v_anio,v_ciclo,v_esquema);
        
        /*****Se comienza indentificando el tipo de ingreso del estudiante*********/
        SELECT  BE.TIPO_DE_INGRESO
        INTO    v_tipo_estudiante
        FROM    B_ESTUDIANTES BE 
        WHERE   BE.CODIGO=p_codigo;
        
        IF v_tipo_estudiante='RA' THEN
            SELECT PKG_CTI_MATRICULA_RA.GETOFERTA(p_codigo,v_anio,v_ciclo, v_esquema)
            INTO   oferta_refcur
            FROM   DUAL;
        ...
        loop
            ...
            if oferta_refcur%notfound and j_of is not null then
    ...
    begin
        ...
        v_query :=
            ...
            'and bp.anio = :anio ' ||
            'and bp.ciclo = :ciclo ' ||
            'and pkg_utils.evaluar_pago(bp.indicador_pago) in (1, 2) ' ||
        ...
        close oferta_refcur;
        if j_of is not null and v_grs is not null then
            json.put(j_of,'grupos',v_grs.to_json_value);
            json_list.append(v_list,j_of.to_json_value);
        else
            v_list := json_list();
    ...
    begin
        ...
        json.put(j_est,'fecha_pivote',pkg_utils.toChar(v_fecha_pivote));
        json.put(j_est,'fecha_inicio',pkg_utils.toChar(v_fecha_inicio));
        json.put(j_est,'fecha_fin',pkg_utils.toChar(v_fecha_fin));
    ...
    begin
        --raise_application_error(-20008, 'Actualmente se esta realizando el cierre de postgrados, por favor intente mas tarde.');
        pkg_html.corsHeaders();
        v_json := p_arg;
        j_arg := json(v_json);
        ...
        --Entre si es RA o inscripcion por bolsa
            ...
            if nvl(n_bolsa, '0') <= 0 then
                begin
                    execute immediate
                        q'!select m.creditos        
                           from b_estudiantes e
                           inner join a_materias m on m.codigo_facultad = e.codigo_facultad
                                           and m.jornada_facultad = e.jornada_facultad
                                           and m.plan_estudio = e.plan_estudio
                                           and m.codigo = :mPlan
                                           and e.codigo = :codigoEst!'
                    into n_cred
                    using p_mplan, p_codigo;
                exception
                when others then
                htp.prn('Bolsa' || n_bolsa);
            ...
            execute immediate
                'insert into ' || v_esquema || '.b_prematricula ' ||
                'select e.codigo, e.codigo_facultad, :mplan, h.codigo_facultad, h.codigo_materia, h.grupo_materia, e.jornada_facultad, null, sysdate, null, null, e.indicador_pago, null, e.anio, e.ciclo, e.codmil, null, e.nombre, null, h.consecutivo ' ||
                'from ' || v_esquema || '.b_estudiantes e, ' ||
                'a_materias m ' ||
                'inner join ' ||
                v_esquema || '.a_horario_horizontal h ' ||
                ...
                'and e.ciclo = :ciclo ' ||
                'and pkg_utils.evaluar_pago(e.indicador_pago)in(1, 2)'
            using p_mplan, p_codigo, p_consecutivo, v_anio, v_ciclo;
            if sql%rowcount != 1 then 
                htp.prn(p_arg);
                htp.prn(p_mplan || '-');
                htp.prn(p_codigo || '-');
                htp.prn(p_consecutivo);
                raise_application_error(-20011, 'No se logro registrar la asignatura a RA');
            end if;
            --Si el grupo está cerrado, se hace la inserción a la tabla de notas parciales
            insert into b_prematricula_notas_depurada(
                codigo_estudiante,
        ...
        begin
            execute immediate
                q'!select codigo_materia
                   from   !' || v_esquema || q'!.a_horario_horizontal
                   where  consecutivo = :consecutivo!'
            into v_codigo_materia
            using p_consecutivo;

            if v_codigo_materia in ('DER60') then
                n_cred := '16';
            end if;
        exception when others then
            v_codigo_materia := null;
        end;
        ...
        UPDATE_SEMESTRE_SUP_INF(p_codigo);
        commit;
    ...
    when others then
        rollback;
        json.put(v_respuesta,'status','fail');
        if sqlcode = -20700 then
            json.put(v_respuesta,'mensaje','Sin cupo');
        else
            json.put(v_respuesta,'mensaje',sqlerrm);
        end if;
        json.htp(v_respuesta,false);
    ...
    begin
        ...
        if sql%rowcount != 1 then 
            raise_application_error(-20700, '[' || p_consecutivo || '] ' || (case p_inc when 1 then 'Sin cupo' else 'No se recuperó el cupo' end));
        end if;
    end modificarCupo;
