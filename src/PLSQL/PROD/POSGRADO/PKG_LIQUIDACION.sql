create or replace package body pkg_liquidacion as
    ...
    --FIXME: Agregar restricciones de liquidaci¿n de gu¿as.
    /*
    ...
    */
    ...
    begin
        ...
        if(ADMISIONES.DEBE_ENCUESTA_COVID(p_codigo) = '1') THEN
            raise_application_error(-20104, 'Por favor llenar la encuesta "Encuesta Retorno Seguro II Ciclo Académico 2020" ubicada en el menu de utilidades.');
        END IF;
        ...
        where (e.codigo is null or (e.ciclo_de_ingreso = e.anio || to_number(e.ciclo) and e.indicador_pago = 'V'))
    ...
    procedure liquidar_aspirante(
        ...
        v_codtran VARCHAR2(100) DEFAULT '';
    ...
    begin
        ...
        SELECT UNIQUE T.ANIO, T.CICLO
        INTO V_ANIO, V_CICLO
        FROM B_ESTUDIANTES T;
        SELECT CODIGOTRAN(P_CODIGO, V_ANIO, V_CICLO)
        into v_codtran
        from dual t;
        
        -- NO ES SOLUCION DEFINITIVA. CCOBALEDA 05-MARZO-2020.
        -- *******************************************************************
        -- DEPENDIENDO DE COMO SE LIQUIDE EL ESTUDIANTE DE FORMA SEMESTRAL, SE ASIGNA EL EQUIVALENTE PARA
        -- EL TRIMESTRE 1 O 2.

        -- DESCRIPCION                  SEMESTRE        TRIMESTRE 1     TRIMESTRE 2
        -- Egresados Antiguos           64              80              84              
        -- Egresados Nuevos             63              81              85
        -- Transferencia interna        4               83              87
        -- Transferencia externa        5               83              87    
        -- Reintegro                    3               82              86
        -- Reintegro de Actualizacion   13              82              86
        -- Movilidad Entrante           21              78              79

        IF V_CODTRAN IN ('03','13') THEN
            IF P_PERIODO = 1 THEN COD_TX := 82; END IF;
            IF P_PERIODO = 2 THEN COD_TX := 86; END IF;
        ELSIF V_CODTRAN IN ('04','05') THEN
            IF P_PERIODO = 1 THEN COD_TX := 83; END IF;
            IF P_PERIODO = 2 THEN COD_TX := 87; END IF;
        ELSIF PKG_DESCUENTOS.ES_ULTIMA_MATERIA(P_CODIGO) = 1 THEN
            IF P_PERIODO = 1 THEN COD_TX := 78; END IF;
            IF P_PERIODO = 2 THEN COD_TX := 79; END IF;
        ELSIF PKG_DESCUENTOS.ES_EGRESADO_NUEVO(P_CODIGO) = 1 THEN
            IF P_PERIODO = 0 THEN COD_TX := 63; END IF;
            IF P_PERIODO = 1 THEN COD_TX := 81; END IF;
            IF P_PERIODO = 2 THEN COD_TX := 85; END IF;
        ELSIF PKG_DESCUENTOS.ES_EGRESADO_ANTIGUO(P_CODIGO) = 1 THEN
            IF P_PERIODO = 0 THEN COD_TX := 64; END IF;
            IF P_PERIODO = 1 THEN COD_TX := 80; END IF;
            IF P_PERIODO = 2 THEN COD_TX := 84; END IF;
        ELSIF V_CODTRAN ='21' THEN
            IF P_PERIODO = 1 THEN COD_TX := 78; END IF;
            IF P_PERIODO = 2 THEN COD_TX := 79; END IF;
        END IF;
        -- *******************************************************************
    ...
    BEGIN
        ...
        IF PUEDEEMITIRGUIADEPAGO(P_CODIGO) = -6 THEN
            RAISE_APPLICATION_ERROR(-20109,'Estudiante con pago registrado en el sistema acad¿mico.');
        ...
        SELECT UNIQUE T.ANIO, T.CICLO
        INTO V_ANIO, V_CICLO
        FROM CACTUALPOS.B_ESTUDIANTES T;
        SELECT CODIGOTRAN(P_CODIGO, V_ANIO, V_CICLO)
        into v_codtran
        from dual t;
          --Se consulta el codigo de transaccion que corresponde segun seleccion.
    ...
    begin
        begin
            ...
            from (
                ...
                union
                select p.indicador_pago, trim(to_char(d.codigo_de_transaccion, '00')) from cti_descuento d inner join cti_periodo p on d.id_periodo = p.id_periodo
                union
        ...
        exception
        when no_data_found then
            raise_application_error(-20200, 'Transacci¿n no permitida: ' || p_cod_transac);
        end;
        --PAGO
        if p_ind_1 = 'X' and p_ind_2 = 'P' then
            ...
            if n_n > 0 then
                --Crea al estudiante de postgrado sin inscripci¿n de materias.
            ...
            /*if sql%rowcount != 1 then
                raise_application_error(-20200, 'No se actualiz¿ el indicador de pago del estudiante: ' || p_codigo_est);
            end if;*/
            select count(*)
            into n_n
    ...
    procedure listadoPeriodos(
        ...
        v_c_2 number default 0;
    begin
        owa_util.mime_header('application/json',false,'utf-8');
        owa_util.http_header_close;
                
        if not regexp_like(p_codigo, '^[A-Z0-9]{2}[0-9]{2}(1|2){1}[0-9]{3}$') then
            raise_application_error(-20300, 'C¿digo no valido.');
        end if;
        begin
        ...
        when no_data_found then
            ...
            where a.cod_def = p_codigo;
            if n_aspirante <= 0 then
                raise_application_error(-20300, 'C¿digo inexistente.');
            end if;
        ...
        elsif ((n_periodo_act = 2 and v_indicador_pago = 'X') or (n_id_periodo = 1 and n_periodo_act = 2) or (n_id_periodo = 1 and n_periodo_act = 1)) and (PuedeGenerarGuiaIITrimestre(p_codigo) = 1) then
        --elsif ((n_periodo_act = 2 and v_indicador_pago = 'X') or (n_id_periodo = 1 and n_periodo_act = 2) or (n_id_periodo = 1 and n_periodo_act = 1)) and (PuedeGenerarGuiaIITrimestre(p_codigo) = 1 or p_codigo = '79182210') then
            open c_periodos for
                select pp.id_periodo, pp.periodo
                from cti_periodo pp
        ...
        close c_periodos;
        --Determina el numero de gu¿as activas sin pago del ciclo actual
        select count(*)
        into n_guias
    ...
    BEGIN
        ...
        --mariano rua mejia 10/01/2020 --acompa¿amiento tutorial
        SELECT COUNT(*)
        into  v_acomp_tut
    ...
    FUNCTION GET_DESCUENTO(P_CODIGO_ESTUDIANTE b_estudiantes.codigo%type, p_periodo cti_periodo.id_periodo%type) RETURN NUMBER IS
    ...
    FUNCTION PUEDEGENERARGUIAIITRIMESTRE(P_CODIGO_ESTUDIANTE B_ESTUDIANTES.CODIGO%TYPE)
    RETURN NUMBER
    IS
        V_CODIGO_FAC A_FACULTADES.CODIGO%TYPE;
        V_EXPREGULAR ADMISIONES.CTI_PARAMETROS.VALOR%TYPE;
    BEGIN
        SELECT A.CODIGO_FACULTAD 
        INTO   V_CODIGO_FAC 
        FROM   (SELECT CODIGO,
                       CODIGO_FACULTAD
                FROM   B_ESTUDIANTES
                ...
                SELECT COD_DEF,
                       CODIGO_FACULTAD
                FROM A_ASPIRANTES) A
        WHERE  A.CODIGO = P_CODIGO_ESTUDIANTE;
        
        SELECT VALOR 
        INTO   V_EXPREGULAR
        FROM   ADMISIONES.CTI_PARAMETROS 
        WHERE  LLAVE = 'bloqueo_programas_generar_guias_trimestres_II';

        IF REGEXP_LIKE(V_CODIGO_FAC, V_EXPREGULAR) THEN
            RETURN 0;
        ELSE
            RETURN 1;
        END IF;
