...
when no_data_found then
  raise_application_error(-20999, 'Sesión no valida. Cierre la ventana y acceda nuevamente.');
when others then
  raise;
end p_leer_cookie;
...
begin
  ...
  for est in (
    ...
    if to_number(est.ciclo_de_ingreso) > to_number(est.anio || to_number(est.ciclo)) then
        htp.prn('"ciclo":"' || est.ciclo_de_ingreso || '",'); --02JUN2020 SE AGREGO UNA COMA
    else
...
begin
    ...
    if p_activas = 1 then
        if p_tipo = 1 then
            ...
            open facus for select distinct f.codigo from a_facultades f where f.activa in ('S') and codigo > '71' and codigo not in ('DE','DA','ET');
        elsif p_tipo = 3 then
            open facus for select distinct f.codigo from a_facultades f where f.activa in ('S') and codigo in ('46');
        elsif p_tipo = 4 then
            open facus for select distinct f.codigo from a_facultades f where f.activa in ('S') and codigo in ('DE','DA','ET');
        else
        ...
        if p_tipo = 1 then
            ...
            open facus for select distinct f.codigo from a_facultades f where f.indicador in ('S') and codigo > '71' and codigo not in ('DE','DA','ET');
        elsif p_tipo = 3 then
            open facus for select distinct f.codigo from a_facultades f where f.indicador in ('S') and codigo in ('46');
        elsif p_tipo = 4 then
            open facus for select distinct f.codigo from a_facultades f where f.indicador in ('S') and codigo in ('DE','DA','ET');
        else
        ...
        if p_tipo = 1 then
            ...
            open facus for select distinct f.codigo from a_facultades f where codigo > '71' and codigo not in ('DE','DA','ET');
        elsif p_tipo = 3 then
            open facus for select distinct f.codigo from a_facultades f where codigo in ('46');
        elsif p_tipo = 4 then
            open facus for select distinct f.codigo from a_facultades f where codigo in ('DE','DA','ET');
        else
...
begin
    ...
    if p_round >= 0 then
        ...
        -- 13/07/2018 (jdcarranza) Según solicitud de la OAR y por el estatuto todos los promedios se muestran con 1 entero 1 decimal.
...
is
    ...
    v_plan a_notas.ind_hnvoplan%type;
begin
    select count( *)
    into n_es_graduado
    ...
    and e.codigo                = p_codigo;
    begin
        select distinct n.ind_hnvoplan
        into v_plan
        from a_notas n
    ...
    if n_es_graduado > 0 then
        return(promedioPeriodoSTD(p_codigo, p_anio, p_ciclo, v_plan, 1));
    else
        return(promedioPeriodoSTD(p_codigo, p_anio, p_ciclo, v_plan, 0));
    end if;
exception
when others then
...
function promedioPonderadoHasta (
    ...
    mant a_materias.codigo%type default 'zzz';
    ...
    if v_plan >= 3 then
        for nota in (
            ...
            b_estudiantes e
                inner join
            a_notas n
                on e.codigo = n.codigo_estudiante and n.ind_hnvoplan = to_char(v_plan)
                inner join
            a_materias m
                on m.codigo = n.codigo_materia 
            ...
            --17/07/2018 (jdcarranza) se omiten materias que provienen de reintegro por actualización
            and concat(n.ano,n.ciclo) not in (SELECT concat(he.anio,DECODE(he.ciclo,'01','01','02','03')) as aniociclo FROM historico_estudiantes he WHERE he.codigo=p_codigo AND he.tipo_de_ingreso = 'RA' AND he.INDICADOR_PAGO IN ('P','V'))
            union all
            select n.codigo_materia mplan,
                m.creditos cr,
                n.valor n,
            ...
            and to_number(n.ano || to_number(regexp_replace(n.ciclo, '[^0-9]+', '0'))) <= to_number(p_anio || to_number(p_ciclo))
            --17/07/2018 (jdcarranza) se omiten materias que provienen de reintegro por actualización
            and concat(n.ano,n.ciclo) not in (SELECT concat(he.anio,DECODE(he.ciclo,'01','01','02','03')) as aniociclo FROM postgrado.historico_estudiantes he WHERE he.codigo=p_codigo AND he.tipo_de_ingreso = 'RA' AND he.INDICADOR_PAGO IN ('P','V'))
            union all
            select n.codigo_materia mplan,
                m.creditos cr,
                n.valor n,
            ...
            if mant != nota.mplan then
                mant := nota.mplan;
                if (nota.i != 'V' and nota.n >= 3) or (nota.i = 'V' and nota.n >= 3.5) then
                    pass := 1;
                else
                    pass := 0;
                end if;
                sumnota := sumnota + (nota.cr * nota.n);
                sumcred := sumcred + nota.cr;
            elsif pass != 1 then
                sumnota := sumnota + (nota.cr * nota.n);
                sumcred := sumcred + nota.cr;
            end if;
        ...
        for nota in (
            ...
            a_notas n
            where n.ciclo not in ('00')
            and (n.ind_hnvoplan = to_char(v_plan) or n.ind_hnvoplan = (case v_plan when 1 then 'N' when 0 then 'A' else 'NOMATCH' end))
            and n.codigo_estudiante = p_codigo
            and to_number(n.ano || to_number(regexp_replace(n.ciclo, '[^0-9]+', '0'))) <= to_number(p_anio || to_number(p_ciclo))
            --17/07/2018 (jdcarranza) se omiten materias que provienen de reintegro por actualización
            and concat(n.ano,n.ciclo) not in (SELECT concat(he.anio,DECODE(he.ciclo,'01','01','02','03')) as aniociclo FROM historico_estudiantes he WHERE he.codigo=p_codigo AND he.tipo_de_ingreso = 'RA' AND he.INDICADOR_PAGO IN ('P','V'))
...
begin
    ...
    if v_anio || v_ciclo = p_anio || p_ciclo then
        if v_esquema = 'admisiones' then
                ...
                select sum(to_number(m.creditos)), min(to_number(m.semestre))
                into p_crd_ins, p_sem_inf
                from postgrado.b_estudiantes e inner join postgrado.b_prematricula p on e.codigo = p.codigo_estudiante and e.anio = p.anio and e.ciclo = p.ciclo
                inner join postgrado.a_materias m on m.codigo = p.materia_plan and m.codigo_facultad = e.codigo_facultad and m.jornada_facultad = e.jornada_facultad and e.plan_estudio = m.plan_estudio
                where m.semestre > 0
                and p.indicador_reglamento is null
                and e.codigo = p_codigo;
                select
                ...
                into p_crd_max
                from
                    postgrado.creditosxsemestre cs
                        inner join
                    postgrado.b_estudiantes e
                        on cs.codigo_facultad = e.codigo_facultad and cs.jornada_facultad = e.jornada_facultad and cs.plan_estudio = e.plan_estudio
                where
                ...
                select sum(to_number(m.creditos)), min(to_number(m.semestre))
                into p_crd_ins, p_sem_inf
                from postgrado.b_estudiantes e inner join cactualpos.b_prematricula p on e.codigo = p.codigo_estudiante and e.anio = p.anio and e.ciclo = p.ciclo
                inner join postgrado.a_materias m on m.codigo = p.materia_plan and m.codigo_facultad = e.codigo_facultad and m.jornada_facultad = e.jornada_facultad and e.plan_estudio = m.plan_estudio
                where m.semestre > 0
                and p.indicador_reglamento is null
                and e.codigo = p_codigo;
        ...
        where
            ...
            and e.ciclo = p_ciclo
            and cs.semestre = p_sem_inf;
        --TODO: Implementar lógica para los DP de periodos anteriores
    end if;
exception
when no_data_found then
    p_sem_inf := 0;
...
begin
    ...
    select he.codigo, he.anio, he.ciclo from historico_estudiantes he where he.codigo = p_codigo and he.materias_pendientes = 0 
    UNION
    SELECT HE.CODIGO, HE.ANIO, HE.CICLO FROM b_ESTUDIANTES HE WHERE HE.CODIGO = p_codigo AND HE.MATERIAS_PENDIENTES = 0 --mariano rua mejia 30/01/2020 caso homologacion en el ciclo en curso  30122650
    order by anio, ciclo) x
    where rownum <= 1)
