...
/*REENVIAR_PETICION EN LOTE*/

DECLARE BEGIN
    FOR V_INTERESADO IN (SELECT INTERESADO        NUMDOC,
                                TIPDOC,
                                CODIGO_PROGRAMA   CODIGO_FACULTAD,
                                JORNADA           JORNADA_FACULTAD
                         FROM CTI_ESTADO_ENVIO_CRM
                         WHERE (FECHA_CREACION_INTERESADO > TO_DATE ('2020-05-21 16:03', 'YYYY-MM-DD HH24:MI')
                                OR LAST_REQUEST_SIMPLE_DATE IS NOT NULL)
                               AND ANIO             = '2020'
                               AND CICLO            = '02'
                               AND ULTIMA_PETICION  IN ('FALLIDA', 'NO ENVIADO')
                        ) LOOP
        DESARROLLOSPRE.PKG_INTERESADOS_CRM.FORCE_UPDATE (V_INTERESADO.TIPDOC, V_INTERESADO.NUMDOC, V_INTERESADO.CODIGO_FACULTAD, V_INTERESADO.JORNADA_FACULTAD, '2020',
                                           '02');
    END LOOP;
END;
/
...
CALL DESARROLLOSPRE.PKG_INTERESADOS_CRM.FORCE_UPDATE ('C', '1016013390', 'ES', 'N', '2020', '02');
...
SELECT INTERESADO,
       ...
       JORNADA,
       NOMBRE,
       FECHA_CREACION_SIMPLIFICADA,
       ANIO,
       ULTIMA_PETICION,
       RECIBIDO_CRM,
       LAST_REQUEST_SIMPLE_DATE,
       LAST_SENDED_CONTENT,
       LAST_SENDED_RESPONSE
FROM CTI_ESTADO_ENVIO_CRM
WHERE (FECHA_CREACION_INTERESADO > TO_DATE ('2020-05-21 16:03', 'YYYY-MM-DD HH24:MI') OR LAST_REQUEST_SIMPLE_DATE IS NOT NULL)
      AND ANIO   = '2020'
      AND CICLO  = '02' 
      AND RECIBIDO_CRM = 'SI'
ORDER BY FECHA_CREACION_INTERESADO DESC;

/* RESUMEN ESTADO DE ENVIO. */

SELECT A.ULTIMA_PETICION, B.NOMBRE CAMPANIA, B.CODIGO,
       COUNT (A.INTERESADO) NUMERO_DE_INTERESADOS
FROM CTI_ESTADO_ENVIO_CRM A
INNER JOIN DESARROLLOSPRE.CTI_CAMPANIA B ON A.ORIGEN = B.CODIGO
WHERE (A.FECHA_CREACION_INTERESADO > TO_DATE ('2020-05-21 16:03', 'YYYY-MM-DD HH24:MI')
       OR A.LAST_REQUEST_SIMPLE_DATE IS NOT NULL)
      AND A.ANIO   = '2020'
      AND A.CICLO  = '02'
GROUP BY 
         A.ULTIMA_PETICION, B.NOMBRE, B.CODIGO
ORDER BY 1, 2, 3;

/* PETICIONES. */

SELECT *
FROM HTTP_REQUEST_LOG_CRM
...
SELECT *
FROM EXCEPTION_LOG_SIMPLE_VIEW
WHERE TRUNC (TRIGGERING_DATE) = TRUNC (SYSDATE);

/*LOG DE RECEPCION DE INTERESADOS*/

SELECT COUNT(*)
FROM CTI_INTERESADOS_CRM_LOG;

/*
SCRIPT ELIMINACION CUIDADO.

DELETE FROM CTI_INTERESADO_CRM WHERE NUMDOC = '48695562';
DELETE FROM CTI_INTERESADO WHERE NUMDOC = '48695562';
DELETE FROM CTI_INTERESADOS_CRM_LOG WHERE NUMDOC = '48695562';
*/
