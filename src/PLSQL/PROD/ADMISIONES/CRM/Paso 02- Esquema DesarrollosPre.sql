...
DECLARE
    ...
    V_ORIGEN_ES_CRM    NUMBER DEFAULT '0';
BEGIN
    SELECT INTERESADO.TIPDOC,
           ...
           CASE WHEN CAMPANIA.CODIGO_PADRE IS NOT NULL THEN CAMPANIA.CODIGO_PADRE ELSE INTERESADO.ORIGEN END ORIGEN,
           INTERESADO.ANIO,
           INTERESADO.CICLO,
           INTERESADO.FECHA
    INTO V_INTERESADO
    ...
    LEFT JOIN CTI_CAMPANIA CAMPANIA ON CAMPANIA.CODIGO = INTERESADO.ORIGEN
    ...
    SELECT COUNT(*)
    INTO V_ORIGEN_ES_CRM
    FROM ADMISIONES.CTI_INTERESADOS_CRM_LOG INTERESADO
    WHERE INTERESADO.TIPDOC = :NEW.TIPDOC
          AND INTERESADO.NUMDOC            = :NEW.NUMDOC
          AND INTERESADO.CODIGO_FACULTAD   = :NEW.CODIGO_FACULTAD
          AND INTERESADO.JORNADA_FACULTAD  = :NEW.JORNADA_FACULTAD
    ...
    V_ADITIONAL_INFO := ADMISIONES.CRM_ADITIONAL_INFO (:NEW.HABEAS_DATA, :NEW.PAGO_INSCRIPCION, :NEW.ENTREVISTA, :NEW.ADMITIDO, :NEW.MATRICULADO,
                              :NEW.SPP, :NEW.FORMULARIO1, :NEW.FORMULARIO2, CASE WHEN UPDATING THEN 0 ELSE 1 END);

    -- SI EL ORIGEN ES CRM SOLO SE ENVIAN ACTUALIZACIONES.
    IF(V_ORIGEN_ES_CRM = '0' OR (V_ORIGEN_ES_CRM != '0' AND UPDATING)) THEN
...
CREATE OR REPLACE PACKAGE BODY PKG_INTERESADOS_CRM IS
    ...
    PROCEDURE FORCE_UPDATE (
        ...
        V_INTERESADO       DESARROLLOSPRE.CTI_INTERESADO%ROWTYPE;
        V_INTERESADO_CRM   DESARROLLOSPRE.CTI_INTERESADO_CRM%ROWTYPE;
        V_ADITIONAL_INFO   ADMISIONES.CRM_ADITIONAL_INFO;
        V_ORIGEN_ES_CRM    NUMBER DEFAULT '0';
    BEGIN
        REFRESH_INTERESADO(P_TIPDOC, P_NUMDOC, P_CODIGO_FACULTAD, P_JORNADA_FACULTAD, P_ANIO, P_CICLO);
        ...
        SELECT *
        INTO V_INTERESADO_CRM
        FROM CTI_INTERESADO_CRM
        WHERE CTI_INTERESADO_CRM.TIPDOC = P_TIPDOC
              AND CTI_INTERESADO_CRM.NUMDOC            = P_NUMDOC
              AND CTI_INTERESADO_CRM.CODIGO_FACULTAD   = P_CODIGO_FACULTAD
              AND CTI_INTERESADO_CRM.JORNADA_FACULTAD  = P_JORNADA_FACULTAD
              AND CTI_INTERESADO_CRM.ANIO              = P_ANIO
              AND CTI_INTERESADO_CRM.CICLO             = P_CICLO;
        ...
        V_ADITIONAL_INFO := ADMISIONES.CRM_ADITIONAL_INFO (V_INTERESADO_CRM.HABEAS_DATA, V_INTERESADO_CRM.PAGO_INSCRIPCION, V_INTERESADO_CRM.ENTREVISTA, V_INTERESADO_CRM.ADMITIDO, V_INTERESADO_CRM.MATRICULADO,
                                  V_INTERESADO_CRM.SPP, V_INTERESADO_CRM.FORMULARIO1, V_INTERESADO_CRM.FORMULARIO2, 1);
    
        IF (V_ORIGEN_ES_CRM != '0') THEN
