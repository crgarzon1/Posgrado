...
DROP TRIGGER UPDATE_CRM_PAGO;

DROP TRIGGER UPDATE_CRM_ESTUDIANTE;

DROP TRIGGER UPDATE_CRM_SPP;
...
COMPOUND TRIGGER
    ...
    AFTER STATEMENT IS BEGIN
        FOR INDX IN 1..G_ROW_LEVEL_INFO.COUNT LOOP
            DESARROLLOSPRE.PKG_INTERESADOS_CRM.REFRESH_INTERESADO (G_ROW_LEVEL_INFO (INDX).TIPDOC, G_ROW_LEVEL_INFO (INDX).NUMDOC, G_ROW_LEVEL_INFO (INDX).CODIGO_FACULTAD, G_ROW_LEVEL_INFO (INDX).JORNADA_FACULTAD, G_ROW_LEVEL_INFO (INDX).ANIO,
...
CREATE OR REPLACE TRIGGER UPDATE_CRM_PAGO FOR
    UPDATE OR INSERT ON G_OTROS_PAGOS
COMPOUND TRIGGER
    TYPE CTI_INTERESADO_TMP IS RECORD (
    ...
    G_ROW_LEVEL_INFO CTI_INTERESADOS_TMP;
    AFTER EACH ROW IS BEGIN
        G_ROW_LEVEL_INFO (G_ROW_LEVEL_INFO.COUNT + 1).CODIGO_EST := :NEW.CODIGO_EST;
    END AFTER EACH ROW;
    AFTER STATEMENT IS
        V_TIPDOC             VARCHAR2 (8);
    ...
    BEGIN
        FOR INDX IN 1..G_ROW_LEVEL_INFO.COUNT LOOP
            BEGIN
                ...
                WHERE ASPIRANTE.CODIGO = G_ROW_LEVEL_INFO (INDX).CODIGO_EST;
...
CREATE OR REPLACE TRIGGER UPDATE_CRM_SPP FOR
    UPDATE OR INSERT ON BENEFICIARIOS_BECAS
COMPOUND TRIGGER
    TYPE CTI_INTERESADO_TMP IS RECORD (
        DOCUMENTO	VARCHAR2(15 BYTE),
    ...
    G_ROW_LEVEL_INFO CTI_INTERESADOS_TMP;
    AFTER EACH ROW IS BEGIN
        G_ROW_LEVEL_INFO (G_ROW_LEVEL_INFO.COUNT + 1).DOCUMENTO := :NEW.DOCUMENTO;
        G_ROW_LEVEL_INFO (G_ROW_LEVEL_INFO.COUNT).ANIO := :NEW.ANIO;
        G_ROW_LEVEL_INFO (G_ROW_LEVEL_INFO.COUNT).CICLO := :NEW.CICLO;
    END AFTER EACH ROW;
    AFTER STATEMENT IS
    ...
    BEGIN
        FOR INDX IN 1..G_ROW_LEVEL_INFO.COUNT LOOP
            BEGIN
                ...
                WHERE ASPIRANTE.NUMDOC = G_ROW_LEVEL_INFO (INDX).DOCUMENTO
                AND ASPIRANTE.ANIO = G_ROW_LEVEL_INFO (INDX).ANIO
                AND ASPIRANTE.CICLO = G_ROW_LEVEL_INFO (INDX).CICLO;
